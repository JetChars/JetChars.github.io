...
- ``/etc/default/docker`` -- Docker Upstart and SysVinit configuration file(ubuntu/debian)
    - location of docker binary, daemon startup opts, proxy, tmpdir
    - can work with export or any other cmds

.. code-block:: shell
    
    sudo echo "DOCKER_OPTS=\"--insecure-registry <registry>:<port> -D\""
    service docker stop && rm -rf /var/lib/docker && mkdir -p /home/docker/var/docker && ln -sf /home/docker/var/docker /var/lib/docker && service docker start
...
ADD               copies new files, zip files(auto decompress), dirs or file URLs to container
...
- FROM --> MAINTAINER --> ENV --> RUN(unify multi RUNs, ``yum clean all`` after ``install`` ) --> COPY/ADD --> CMD
